# scraper_articles_poc.py
# Proof of concept: Scrape articles from page 11 only
...
def get_article_links(page_num):
    ...
    try:
        ...
        if not link_elements:
            print(f"  ⚠ No links found with selector '{ARTICLE_LINK_SELECTOR}'")
            print(f"     Try running explore_structure.py to identify correct selectors")
            return []
        ...
        for link_el in link_elements:
            ...
            if parent and parent.name == 'p' and 'article-title' in parent.get('class', []):
                continue
        ...
        print(f"  ✓ Found {len(links)} unique article link(s)")
        return links
...
def parse_article(url):
    ...
    try:
        ...
        article_id = "".join(c for c in article_id if c.isalnum() or c in ('-', '_'))[:50]
        ...
        if not body_text:
            print(f"  ⚠ No body content found with selector '{ARTICLE_BODY_SELECTOR}'")
        ...
        if body_text:
            print(f"  ✓ Extracted: title={bool(title)}, text_length={len(body_text)} chars")
        else:
...
def main():
    ...
    print("\n" + "="*60)
    print("ARTICLE SCRAPER - PROOF OF CONCEPT")
    print("="*60)
    print(f"\nScraping articles from page {ARTICLE_LIST_POC_PAGE} only")
    print(f"Output: {os.path.join(PROCESSED_DIR, 'articles_poc.jsonl')}")
    ...
    # Get article links from page 11
    article_urls = get_article_links(ARTICLE_LIST_POC_PAGE)
    
    if not article_urls:
        print("\n✗ No article URLs found. Please:")
        print("  1. Run explore_structure.py to identify correct selectors")
        print("  2. Update ARTICLE_LINK_SELECTOR in config.py")
        return
    
    # Filter out already completed
    new_urls = [url for url in article_urls if url not in completed_urls]
    if new_urls:
    ...
    # Scrape articles
    output_path = os.path.join(PROCESSED_DIR, "articles_poc.jsonl")
    success_count = 0
    fail_count = 0
    ...
    with open(output_path, "a", encoding="utf-8") as out_f:
        for url in new_urls:
            ...
            if doc and doc.get("clean_text"):
                ...
                fail_count += 1
                print(f"  ✗ Skipping article (no content extracted)")
