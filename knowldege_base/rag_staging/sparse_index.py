"""
Sparse BM25 index over KB chunks (staging).

Uses `rank_bm25` with a simple Arabic-aware tokenizer.

Usage (from project root, venv activated):

    python -m knowldege_base.rag_staging.sparse_index "ما هو الاكتئاب؟"
"""
...
class SparseKBIndex:
    ...
    This is fine for the current scale (~2.6k chunks). If needed,
    we can later persist the index or move to a dedicated search engine.
    """
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m knowldege_base.rag_staging.sparse_index \"<arabic query>\"")
        sys.exit(1)

    query = sys.argv[1]
    print(f"Building sparse index over KB chunks...")
    index = SparseKBIndex.build()
    print(f"Index built with {len(index.chunks)} chunks.")

    results = index.search(query, top_k=5)
    print(f"\nTop 5 results for query: {query!r}\n")
